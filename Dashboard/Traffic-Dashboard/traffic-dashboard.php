...
$moduleSuffixesIn  = ['_ifHCIInOctets', '_ifHCInOctets', 'ifHCInOctets', '_ifInOctets', 'ifInOctets', ' - traffic in', ' - traffic_in', '_traffic_in', ' traffic in', ' - in octets', '_in_octets'];
$moduleSuffixesOut = ['_ifHCOutOctets', '_ifHCIOutOctets', 'ifHCOutOctets', '_ifOutOctets', 'ifOutOctets', ' - traffic out', ' - traffic_out', '_traffic_out', ' traffic out', ' - out octets', '_out_octets'];
$moduleSuffixesSpeed = ['_ifHighSpeed', 'ifHighSpeed', '_ifSpeed', 'ifSpeed', ' - Speed', '- Speed', '_Speed', ' - ifspeed', '_ifspeed']; 
$modulePrefixesSpeed = ['ifhighspeed - ', 'ifhighspeed-', 'ifspeed - ', 'ifspeed-', 'speed - ', 'speed-'];
$modulePrefixesIn  = ['traffic in - ', 'traffic in-', 'traffic_in_', 'traffic in ', 'in traffic - ', 'in traffic ', 'trafficin_'];
$modulePrefixesOut = ['traffic out - ', 'traffic out-', 'traffic_out_', 'traffic out ', 'out traffic - ', 'out traffic ', 'trafficout_'];
...
if ($api === 'save_config') {
    ...
    if ($bytes === false) {
        ...
        if (file_exists($alt_file)) {
            @file_put_contents($alt_file, $input);
        }
...
if ($api === 'export') {
    try {
        ...
        foreach ($node_agent_ids as $node => $aids) {
            ...
            // Check if tcategory and am.id_category exist
            $has_category = false;
            try {
                $stc = $active_pdo->query("SHOW COLUMNS FROM tagente_modulo LIKE 'id_category'");
                if ($stc && $stc->rowCount() > 0) {
                    $stc2 = $active_pdo->query("SHOW TABLES LIKE 'tcategory'");
                    if ($stc2 && $stc2->rowCount() > 0) {
                        $has_category = true;
                    }
                }
            } catch (Throwable $e) {
            ...
            if ($has_category) {
                ...
                $sql = "SELECT a.id_agente, a.alias, a.nombre, a.direccion, am.id_agente_modulo, am.nombre as mod_name, am.unit, ae.datos, 0 AS id_category, '' AS category_name 
                        FROM tagente a
                        JOIN tagente_modulo am ON a.id_agente = am.id_agente
                        LEFT JOIN tagente_estado ae ON am.id_agente_modulo = ae.id_agente_modulo
                        WHERE a.id_agente IN ($placeholders) AND a.disabled = 0 AND am.disabled = 0";
...
if ($api === 'data') {
    ...
    try {
        ...
        foreach ($target_nodes as $node => $info) {
            ...
            try {
                ...
                try {
                    ...
                    if ($stc && $stc->rowCount() > 0) {
                        $stc2 = $active_pdo->query("SHOW TABLES LIKE 'tcategory'");
                        if ($stc2 && $stc2->rowCount() > 0) $has_category = true;
                    }
                } catch (Throwable $e) { $has_category = false; }

                $catSelect = $has_category ? "am.id_category, c.name AS category_name," : "'' AS category_name,";
                $catJoin = $has_category ? "LEFT JOIN tcategory c ON am.id_category = c.id" : "";

                $params = [];
                $likeClauses = [];
                $allPatterns = array_merge(
                    $moduleSuffixesIn, $moduleSuffixesOut, $moduleSuffixesSpeed, 
                    $moduleSuffixesIndex, $moduleSuffixesStatus,
                    $modulePrefixesSpeed, $modulePrefixesIn, $modulePrefixesOut
                );
                foreach ($allPatterns as $i => $pat) {
                    $likeClauses[] = "LOWER(am.nombre) LIKE :lp{$i}";
                    $params[":lp{$i}"] = "%" . strtolower($pat) . "%";
                }
                $allLikes = implode(" OR ", $likeClauses);

                $sql = "SELECT am.id_agente_modulo, am.nombre AS module_name, am.unit AS module_unit, am.descripcion AS description, a.id_agente, a.alias, a.nombre AS agent_name, a.direccion AS ip, {$catSelect} ae.datos AS module_data
                ...
                foreach ($modules as $m) {
                    ...
                    foreach ($modulePrefixesSpeed as $p) { if(str_starts_with($modNameL, strtolower($p))) { $dir='speed'; $base=substr($modName, strlen($p)); break; } }
                    if(!$dir) foreach ($modulePrefixesIn as $p) { if(str_starts_with($modNameL, strtolower($p))) { $dir='in'; $base=substr($modName, strlen($p)); break; } }
                    if(!$dir) foreach ($modulePrefixesOut as $p) { if(str_starts_with($modNameL, strtolower($p))) { $dir='out'; $base=substr($modName, strlen($p)); break; } }
                    if(!$dir) {
                        foreach ($moduleSuffixesIn as $s) {
                            $sL = strtolower($s);
                            $pos = strpos($modNameL, $sL);
        ...
        $interfaces = array_values(array_filter($interfaces, function($i) {
            return (!empty($i['mod_in']) || !empty($i['mod_out']) || !empty($i['mod_speed']) || !empty($i['mod_status']));
        }));

        $enabled_categories = $input['enabled_categories'] ?? null;
        if (!empty($enabled_categories) && is_array($enabled_categories)) {
        ...
        error_log("Traffic Dashboard Error: " . $e->getMessage() . " at " . $e->getFile() . ":" . $e->getLine());
...
<script>
    ...
    async function init() {
        ...
        // URL Params override UI filter defaults
        ...
        let activeUnitsList = null;
        let activeCategoriesList = null;
        if(params.has('enabled_units')) activeUnitsList = params.get('enabled_units').split(',').map(s => s.trim()).filter(Boolean);
        if(params.has('enabled_categories')) activeCategoriesList = params.get('enabled_categories').split(',').map(s => s.trim()).filter(Boolean);
    ...
    function openDashboard(id, isInitial = false) {
        ...
        if (unit) document.getElementById('f_unit').value = unit;
    ...
    function saveSettings() {
        ...
        const settingsObj = { warn, crit, fs, unit, sort, speed_filter, search, show_units, enabled_units, enabled_categories, per_page, refresh };

        // Save to LocalStorage with Dashboard ID as Key
        localStorage.setItem('pfms_settings_' + currentDashId, JSON.stringify(settingsObj));

        // Save to in-memory dashboard object and persist to server JSON file
        const dash = masterDashboards.find(x => x.id === currentDashId);
        if (dash) {
            dash.settings = settingsObj;
            saveConfigToServer();
        }

        updateUnitDropdown(enabled_units);
        if (enabled_units.includes(unit)) {
    ...
    function saveNewDashboard() {
        const name = document.getElementById('m_name').value.trim(); if(!name) return alert("Name required");
        const gsel = document.getElementById('m_group');
        const asel = document.getElementById('m_agent');
        
        const gname = (gsel.selectedIndex >= 0 && gsel.options[gsel.selectedIndex]) ? gsel.options[gsel.selectedIndex].text : 'All Groups';
        const aname = (asel.selectedIndex >= 0 && asel.options[asel.selectedIndex]) ? asel.options[asel.selectedIndex].text : 'All Nodes';
        ...
        if (editId) {
            ...
            if (idx !== -1) {
                ...
                masterDashboards[idx].group_name = gname;
                masterDashboards[idx].agent_id = asel.value;
                masterDashboards[idx].agent_name = aname;
            }
        } else {
            const id = 'dash_'+Math.random().toString(36).substr(2,9);
            masterDashboards.push({ id, name, group_id: gsel.value, group_name: gname, agent_id: asel.value, agent_name: aname, hidden_interfaces: [], settings: {} });
        }
                ...
                if (loaded && typeof loaded === 'object' && !Array.isArray(loaded)) {
                    ...
                    masterDashboards = masterDashboards.map(d => {
                        if (d.id === targetId) {
                            ...
                            d.settings = loaded.settings || d.settings || {};
                        }
                        return d;
                    });
                    saveConfigToServer(() => {
