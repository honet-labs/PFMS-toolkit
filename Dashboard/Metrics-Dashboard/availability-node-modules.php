...
if ($api === 'card_data' && $db_status) {
    ...
    try {
        ...
        foreach ($target_nodes as $node => $info) {
            ...
            if ($search !== '') {
                $whereClause .= " AND (a.alias LIKE ? OR m.nombre LIKE ? OR a.direccion LIKE ? OR g.nombre LIKE ?)";
                $node_params[] = '%' . $search . '%';
                $node_params[] = '%' . $search . '%';
                $node_params[] = '%' . $search . '%';
                $node_params[] = '%' . $search . '%';
            ...
            // 1. Fetch Stats via Group By (Fast)
            $stats_sql = "SELECT COALESCE(te.estado, 4) as estado, COUNT(*) as cnt
                          FROM tagente a 
                          ...
                          GROUP BY COALESCE(te.estado, 4)";
            
            try {
                $stmt_stats = $active_pdo->prepare($stats_sql);
                $stmt_stats->execute($node_params);
                $stats_rows = $stmt_stats->fetchAll(PDO::FETCH_ASSOC);
                foreach ($stats_rows as $sr) {
                    $est = (int)$sr['estado'];
                    $cnt = (int)$sr['cnt'];
                    $stats['total'] += $cnt;
                    if ($est === 0) $stats['normal'] += $cnt;
                    elseif ($est === 1) $stats['critical'] += $cnt;
                    elseif ($est === 2) $stats['warning'] += $cnt;
                    elseif ($est === 4) $stats['not_init'] += $cnt;
                    else $stats['unknown'] += $cnt;
                }
            } catch (Throwable $e) {
                error_log("Error fetching stats for node '{$node}': " . $e->getMessage());
            }

            // 2. Fetch only a limited slice of rows from database (Fast)
        ...
        // Slice for this specific page
        $paginated_data = array_slice($formatted_data, $offset, $limit);
...
async function init() {
    ...
    if (IS_MODAL_ONLY) {
        ...
        let targetCard = loadedCards.find(c => c.id === targetCardId);
        if (!targetCard) {
            targetCard = {
                id: targetCardId || 'std',
                title: p.get('title') || 'Availability',
                group_id: p.get('group_id') || '0',
                keyword: p.get('keyword') || 'Host Alive',
                limit: p.get('limit') || 15,
                refresh_sec: p.get('refresh') || 60,
                view_type: p.get('view_type') || 'table',
                manual_ids: p.get('manual_ids') || '',
                lbl_ok: p.get('lbl_ok') || '',
                lbl_warn: p.get('lbl_warn') || '',
                lbl_crit: p.get('lbl_crit') || ''
            };
        }
...
function updateShareUrls() {
    if (!currentShareCard) return;
    const hideHeader = document.getElementById('shareHideHeaderChk').checked;
    const u = new URL(window.location.origin + window.location.pathname);
    u.searchParams.set('s', '1');
    u.searchParams.set('d', currentShareCard.id);
    if (hideHeader) {
        u.searchParams.set('hide_header', '1');
    }
    const urlString = u.toString();
    const embedCode = `<iframe src="${urlString}" width="100%" height="280" frameborder="0"></iframe>`;
