...
<head>
    ...
    <style>
        ...
        #view_detail { height: 100% !important; width: 100% !important; display: flex !important; flex-direction: column !important; flex: 1 !important; }
        .grid-layout { height: 100% !important; width: 100% !important; display: flex !important; flex-direction: column !important; flex: 1 !important; grid-template-columns: 1fr !important; gap: 0 !important; columns: 1 !important; margin: 0 !important; }
        .dashboard-card { height: 100% !important; width: 100% !important; display: flex !important; flex-direction: column !important; flex: 1 !important; box-shadow: none !important; border: 1px solid #e2e8f0 !important; border-radius: 6px !important; margin: 0 !important; margin-bottom: 0 !important; overflow: hidden !important; }
        .dashboard-card-header { padding: 8px 12px !important; flex-shrink: 0 !important; }
        .dashboard-card-body { padding: 6px 8px 8px 8px !important; gap: 0 !important; display: flex !important; flex-direction: column !important; flex: 1 !important; height: calc(100% - 40px) !important; overflow: hidden !important; }
        .dashboard-card-body > div[id^="content_view_"] { height: 100% !important; width: 100% !important; display: flex !important; flex-direction: column !important; flex: 1 !important; }
...
<div class="modal-overlay" id="builderModal">
    <div class="modal-box">
        ...
        <div style="display:flex; gap:15px; margin-bottom: 15px;">
            <div style="flex:1;">
                <label>Stats Card Font Size (px)</label>
                <input type="number" id="b_stat_font_size" class="form-control-fix" placeholder="22" value="22" min="8" max="100">
            </div>
            <div style="flex:1;">
                <label>Panel Height (px)</label>
                <input type="number" id="b_height" class="form-control-fix" placeholder="e.g. 200 (Auto)" min="80" max="1200">
            </div>
        </div>
...
function renderGrid() {
    ...
    dashboardCards.forEach(c => {
        ...
        const statFs = c.stat_font_size ? `font-size: ${c.stat_font_size}px !important;` : '';
        const statLblFs = c.stat_font_size ? `font-size: ${Math.max(8, Math.round(c.stat_font_size * 0.4))}px !important;` : '';
        const statMinH = (c.height) ? `min-height: ${Math.max(50, parseInt(c.height) - 80)}px;` : '';
        const statCardStyle = `${statBg} ${statMinH} ${c.height ? 'display:flex; flex-direction:column; justify-content:center; align-items:center;' : ''}`;

        let statsHtml = '';
        if (visStats.includes('total')) {
            statsHtml += `<div class="mini-stat st-border-black" style="${statCardStyle}" onclick="showDetailModal('${c.id}', 'all')"><div class="mini-stat-val text-black" style="${statValColor} ${statFs}" id="st_tot_${c.id}">0</div><div class="mini-stat-label" style="${statLblColor} ${statLblFs}">TOTAL</div></div>`;
        }
        if (visStats.includes('normal')) {
            statsHtml += `<div class="mini-stat st-border-green" style="${statCardStyle}" onclick="showDetailModal('${c.id}', 'normal')"><div class="mini-stat-val text-green" style="${statValColor} ${statFs}" id="st_normal_${c.id}">0</div><div class="mini-stat-label" style="${statLblColor} ${statLblFs}">UP</div></div>`;
        }
        if (visStats.includes('warning')) {
            statsHtml += `<div class="mini-stat st-border-yellow" style="${statCardStyle}" onclick="showDetailModal('${c.id}', 'warning')"><div class="mini-stat-val text-yellow" style="${statValColor} ${statFs}" id="st_warning_${c.id}">0</div><div class="mini-stat-label" style="${statLblColor} ${statLblFs}">WARNING</div></div>`;
        }
        if (visStats.includes('critical')) {
            statsHtml += `<div class="mini-stat st-border-red" style="${statCardStyle}" onclick="showDetailModal('${c.id}', 'critical')"><div class="mini-stat-val text-red" style="${statValColor} ${statFs}" id="st_critical_${c.id}">0</div><div class="mini-stat-label" style="${statLblColor} ${statLblFs}">CRITICAL</div></div>`;
        }
        if (visStats.includes('unknown')) {
            statsHtml += `<div class="mini-stat st-border-gray" style="${statCardStyle}" onclick="showDetailModal('${c.id}', 'unknown')"><div class="mini-stat-val text-gray" style="${statValColor} ${statFs}" id="st_unknown_${c.id}">0</div><div class="mini-stat-label" style="${statLblColor} ${statLblFs}">UNKNOWN</div></div>`;
        }
        if (visStats.includes('not_init')) {
            statsHtml += `<div class="mini-stat st-border-blue" style="${statCardStyle}" onclick="showDetailModal('${c.id}', 'not_init')"><div class="mini-stat-val text-blue" style="${statValColor} ${statFs}" id="st_not_init_${c.id}">0</div><div class="mini-stat-label" style="${statLblColor} ${statLblFs}">NOT INIT</div></div>`;
        }
        ...
        pageData.forEach(r => {
            ...
            if (visibleCols.includes('agent')) {
                ...
                if (rawAgentId && rawAgentId != '0' && rawAgentId !== 'undefined') {
                    agentLinkHtml = `<a href="${PANDORA_URL}/index.php?sec=estado&sec2=operation/agentes/ver_agente&id_agente=${rawAgentId}" target="_blank" class="agent-link" style="font-size:${tableFs}px!important; font-family: 'Inter', system-ui, -apple-system, sans-serif !important; color:#1976d2 !important; font-weight:600 !important; text-decoration:none; cursor:pointer;">${r.agent_alias}</a>`;
                } else {
            ...
            if (visibleCols.includes('module')) {
                ...
                const modHtml = (rawModId && rawModId != '0' && rawModId !== 'undefined')
                    ? `<a href="${PANDORA_URL}/index.php?sec=view&sec2=operation%2Fagentes%2Fstatus_monitor&id_module=${rawModId}" target="_blank" class="module-link" style="color:#0b1a26 !important; font-weight:normal; text-decoration:none; cursor:pointer; font-family: 'Inter', system-ui, -apple-system, sans-serif !important;">${r.module_name}</a>`
...
function renderDetailModalPage() {
    ...
    if (pageData.length === 0) {
        ...
        pageData.forEach(r => {
            ...
            let agentLinkHtml = '';
            if (rawAgentId && rawAgentId != '0' && rawAgentId !== 'undefined') {
                agentLinkHtml = `<a href="${PANDORA_URL}/index.php?sec=estado&sec2=operation/agentes/ver_agente&id_agente=${rawAgentId}" target="_blank" class="agent-link" style="color:#1976d2 !important; font-weight:600 !important; text-decoration:none; cursor:pointer;">${r.agent_alias}</a>`;
            } else {
                agentLinkHtml = `<span style="font-weight:500; color:#334155;">${r.agent_alias}</span>`;
            }
...
async function openBuilder() {
    ...
    document.getElementById('b_stat_font_size').value = '22';
    document.getElementById('b_height').value = '';
...
async function openEdit(id) {
    ...
    document.getElementById('b_stat_font_size').value = c.stat_font_size || '22';
    document.getElementById('b_height').value = c.height || '';
...
function saveWidget() {
    ...
    const card = {
        ...
        stat_font_size: parseInt(document.getElementById('b_stat_font_size').value) || 22,
        height: parseInt(document.getElementById('b_height').value) || '',
        ...
        auto_convert_traffic: autoConvertTraffic,
...
function copyStandaloneUrl(card) {
    if (!currentDashId) return;
    currentShareCard = card;
    document.getElementById('shareModalTitle').innerText = 'Share: ' + (card.title || 'Widget');
...
function renderWidgetChart(cardId, viewType, data, chartLimit = 0, stats = {}, history = []) {
    ...
    if (viewType === 'pie' || viewType === 'donut') {
        ...
        if (window.ResizeObserver) {
            const canvasEl = document.getElementById(`chart_canvas_${cardId}`);
            if (canvasEl && (!window._roMap || !window._roMap[cardId])) {
                window._roMap = window._roMap || {};
                window._roMap[cardId] = new ResizeObserver(() => {
                    if (activeCharts[cardId] && typeof activeCharts[cardId].resize === 'function') {
                        activeCharts[cardId].resize();
                    }
                });
                window._roMap[cardId].observe(canvasEl);
            }
        }
    }
}
